...
client.subscribe("notiEmail", async function({ task, taskService }) {
    const tomail = task.variables.get("fromemail");
    const resolvedResult = task.variables.get("giaiphap");
    const message = createEmailMessage(resolvedResult, tomail);

    
    const mailOptions = {
      from:  '21522456@gm.uit.edu.vn', 
      to: tomail,
      subject: "Kết Quả Khiếu Nại",
      text: message,
    }; //nội dung email
